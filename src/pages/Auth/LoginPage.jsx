...
import { loginStartShift } from '../../store/slices/authSlice';
...
export const LoginPage = () => {
  ...
  const handleLoginSubmit = (e) => {
    ...
    if (username === 'morning' && password === '1234') {
      setValidatedUser({ id: 'morning', name: 'Morning Shift' });
      setShowOpeningCash(true);
    } else if (username === 'evening' && password === '1234') {
      setValidatedUser({ id: 'evening', name: 'Evening Shift' });
      setShowOpeningCash(true);
    } else {
          ...
          {!showOpeningCash ? (
            <>
              ...
              <form onSubmit={handleLoginSubmit} className="w-full flex flex-col gap-4">
                ...
                <button 
                  type="submit"
                  className="w-full h-[48px] bg-[#ffb01d] hover:bg-[#e59e1a] text-white rounded-[12px] text-[16px] font-bold mt-4 shadow-[0_4px_12px_rgba(255,176,29,0.3)] transition-all flex items-center justify-center"
                >
                  Sign In
                </button>
